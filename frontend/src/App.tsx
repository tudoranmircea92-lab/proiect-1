...
type ImportanceView = 'controllable' | 'context' | 'combined'
...
export default function App() {
  ...
  const [importanceView, setImportanceView] = useState<ImportanceView>('combined')
  ...
  const filteredFeatureImportance = useMemo(() => {
    ...
    return source.filter((x) => {
      if (importanceView === 'combined') return true
      if (importanceView === 'controllable') return /\.(pwr|m1g|m2g|m3g|s\d+g)$/.test(x.feature)
      return !/\.(pwr|m1g|m2g|m3g|s\d+g)$/.test(x.feature)
    }).slice(0, 20)
  ...
  const compartmentImportanceData = useMemo(() => {
    if ((importance as any)?.by_compartment?.length) return (importance as any).by_compartment
    return scan?.chart_data?.importance_by_compartment ?? []
  }, [importance, scan])

  const shares = useMemo(() => {
    if (importance?.share) return importance.share
    return { controllable: scan?.chart_data?.total_controllable_share ?? 0, context: scan?.chart_data?.total_context_share ?? 1 }
  }, [importance, scan])
  ...
  const onSelectFiles = async (files: File[]) => {
    ...
    if (previewCandidates.length > 0) {
      try {
        ...
        const res = await api.post('/api/train/preview', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
  ...
  const scanDataset = async () => {
    ...
    try {
      ...
      setScanProgress(55)
      const res = await api.post('/api/train/scan', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
      setScan(res.data)
  ...
  const runTraining = async () => {
    ...
    try {
      const res = await api.post('/api/train/run', {
      ...
      setRun(res.data)
      const imp = await api.get(`/api/train/importance/${res.data.run_id}`)
  ...
  const exportProcessedData = async () => {
    ...
    const res = await api.post('/api/train/export-processed', { dataset_paths: datasetPaths, file_format: exportFormat }, { responseType: 'blob' })
  ...
  const loadRuns = async () => setRegistry((await api.get('/api/api/train/runs')).data)

  const activateModel = async (runId: string) => {
    await api.post('/api/train/registry/activate', { run_id: runId })
    await loadRuns()
  }

  const downloadModelArtifact = async (runId: string) => {
    const res = await api.get(`/api/train/artifact/${runId}`, { responseType: 'blob' })
  ...
  const runOptimization = async () => {
    ...
    const res = await api.post('/api/optimize/run', {
          ...
          <div className="col-span-6 rounded-xl bg-slate-50 p-4">
            ...
            <div className="mt-3 flex gap-2">
              <div className="flex gap-1">
                {['combined','controllable','context'].map((v)=><button key={v} className={`rounded px-2 py-1 text-xs ${importanceView===v?'bg-indigo-600 text-white':'bg-slate-300'}`} onClick={() => setImportanceView(v as ImportanceView)}>{v}</button>)}
              </div>
            ...
            {filteredFeatureImportance.length > 0 && <div className="mt-3"><FeatureImportanceChart ref={importanceChartRef} title={`Feature Importances (${importanceView})`} data={filteredFeatureImportance} /></div>}
            {compartmentImportanceData.length > 0 && <div className="mt-3"><FeatureImportanceChart title="Importance by Compartment" data={compartmentImportanceData.map((c:any)=>({feature:c.compartment,importance:c.importance}))} /></div>}
            <div className="mt-2 rounded bg-white p-2 text-xs">Controllable share: {(shares.controllable*100).toFixed(1)}% | Context share: {(shares.context*100).toFixed(1)}%</div>
