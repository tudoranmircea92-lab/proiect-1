...
from backend.app.core.constants import LEAKAGE_COLUMNS, MANDATORY_COLUMN, PLATE_COLUMN
...
def _feature_sets(df: pd.DataFrame, targets: list[str], model_type: str) -> tuple[list[str], list[str], list[str]]:
    excluded = set(targets) | LEAKAGE_COLUMNS | {'plate', 'file_ts', 'date', 'time'}
    included: list[str] = []
    knobs: list[str] = []
    for col in df.columns:
        if col in excluded or col == MANDATORY_COLUMN:
            continue
        if is_knob_column(col):
            included.append(col)
            knobs.append(col)
        elif model_type == 'process':
            included.append(col)
    excluded_cols = [c for c in df.columns if c not in included and c not in targets]
    return sorted(set(included)), sorted(set(excluded_cols)), sorted(set(knobs))
...
def _split_data(df: pd.DataFrame, split_mode: str, ratios: tuple[float, float, float]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    ts_col = 'file_ts' if 'file_ts' in df.columns else ('timestamp' if 'timestamp' in df.columns else None)
    if ts_col:
...
def _importance_payload(features: list[str], importances: np.ndarray) -> dict:
    ...
    for feat, imp in zip(features, importances):
        group = 'controllable' if is_knob_column(feat) else 'context'
...
def train_model(
    ...
    features, excluded, knobs = _feature_sets(scoped_df, target_columns, model_type)
    ...
    write_json(artifact_paths['schema'], {'features': features, 'targets': target_columns, 'knobs': knobs})
    ...
    write_json(
        artifact_paths['train_config'],
        {
            ...
            'merged_rows_count': int(len(scoped_df)),
